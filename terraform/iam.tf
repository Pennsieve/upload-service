...
data "aws_iam_policy_document" "iam_policy_document" {
  statement {
    ...
    resources = [
      data.terraform_remote_state.platform_infrastructure.outputs.upload_chunk_hash_dynamodb_table_arn,
      "${data.terraform_remote_state.platform_infrastructure.outputs.upload_chunk_hash_dynamodb_table_arn}/*",
      data.terraform_remote_state.platform_infrastructure.outputs.upload_preview_metadata_dynamodb_table_arn,
      "${data.terraform_remote_state.platform_infrastructure.outputs.upload_preview_metadata_dynamodb_table_arn}/*",
    ]
  }

  statement {
    sid    = "PreviewFilesDynamoDBPermissions"
    effect = "Allow"
    ...
    resources = [
      data.terraform_remote_state.platform_infrastructure.outputs.upload_preview_files_dynamodb_table_arn,
      "${data.terraform_remote_state.platform_infrastructure.outputs.upload_preview_files_dynamodb_table_arn}/*",
    ]
  }
